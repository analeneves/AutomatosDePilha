...
int RECONHECE_PAR(Pilha *pil) {
	...
	for (i = 0; i < n; i++) {
		...
		else if (cadeia[i] == 0 && pil->prim->info == 1 && q == 0) {
			dado = pop(pil);
			push(pil, 1);
			push(pil, 1);
			imprimePilha(pil);
		...
		else if (cadeia[i] == 1 && pil->prim->info == 1 && q == 0) {
			q = 1;
			dado = pop(pil);
			imprimePilha(pil);
			printf("Estado do autômato: %d\n\n", q);
		}
		else if (cadeia[i] == 1 && pil->prim->info == 1 && q == 1) {
			dado = pop(pil);
			imprimePilha(pil);
			printf("Estado do autômato: %d\n\n", q);
		}
...
void inicio() {	
	...
	//  Simbolo de Palavra Vazia : 2
	printf("Insira a sequência de entrada(2: palavra vazia):\n");
	for (i = 0; i < n; i++) {
		scanf("%d", &cadeia[i]);
	}
	// system("clear");
